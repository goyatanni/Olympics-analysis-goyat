...
def data_over_time(df,col):
    ...
    events_over_time.rename(columns={'Year': 'Edition', 'count': col}, inplace=True)
    return events_over_time

    athlete_over_time = df.drop_duplicates(['Year', col])['Year'].value_counts().reset_index().sort_values('Year')
    athlete_over_time.rename(columns={'Year': 'Edition', 'count': col}, inplace=True)
...
def country_events_heatmap(df,country):
    temp_df = df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
    new_df = temp_df[temp_df['region'] == country]
